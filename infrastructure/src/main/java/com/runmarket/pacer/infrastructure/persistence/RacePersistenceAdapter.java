...
import com.runmarket.pacer.infrastructure.persistence.mapper.RaceMapper;
import com.runmarket.pacer.infrastructure.persistence.repository.RaceJpaRepository;
        ...
        RaceJpaEntity entity = raceJpaRepository.findByExternalId(race.getExternalId())
                .map(existing -> {
                    existing.update(
                            ...
                            race.getDate(), race.getStartTime(),
                            race.getVenue(), race.getVenueAddress(),
                            race.getRegion(), race.getOrganizer(),
                            race.getRepresentative(), race.getPhone(), race.getEmail(),
                            race.getRegistrationStartDate(), race.getRegistrationEndDate(),
                            race.getHomepageUrl(), race.getLat(), race.getLng(),
                            race.getDescription()
                    );
                    return existing;
                })
